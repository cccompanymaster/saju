...
- **다국어**: 기본 **영어**, 한국어 토글(`m/js/i18n.js`). UI·AI 해석·PDF 모두 언어 전환(EN/KO). 요청 시 `lang` 전달.
- **결과 재열람(2주)**: 결제 시 결과를 토큰으로 저장(14일 TTL), `/m/?r=<token>` 으로 재열람. 이메일·알림톡에 링크 포함. (인메모리 — 영속·다중인스턴스는 DB로 교체)
- **카카오 로그인(회원가입)**: `KAKAO_JS_KEY` 설정 시 카카오 로그인 버튼 노출(이메일·닉네임 프리필).
- **카카오 채널 친구추가**: `KAKAO_CHANNEL_ID` 설정 시 채널 추가 버튼 노출(`Kakao.Channel.addChannel` / 폴백 `pf.kakao.com`).
- **카카오 알림톡**: Solapi 대행사 어댑터(`SOLAPI_*`+`KAKAO_PFID`/`KAKAO_TEMPLATE_ID`). 미설정 시 mock. 템플릿은 비즈채널 사전 승인 필요.
